...
public class qpGrid : MonoBehaviour
{
    ...
    public Vector2 startCoordinate = new Vector3(-50,-50);      // Start position from which to ray cast for terrain suitable for nodes.
    public Vector2 endCoordinate = new Vector3(50,50);  // End position from which to ray cast for terrain suitable for nodes.
    ...
    public enum Axis {Y=4, Z=5}; //The Axises
    ...
    public float spread = 1;    // Indicates the distance between each node.
	...
	public void Awake () {  //Generate new nodes with raycast collision detection
    ...
    public void Update()
    {
        if (DrawInEditor) {
            ...
            foreach (qpNode node in allNodes) {
                foreach (qpNode node2 in node.ContactedNodes) {
                    Debug.DrawLine(node.GetCoordinates(), node2.GetCoordinates(), new Color(0,0,1,.7f), 0, true);
                }
            }
        }
    }
